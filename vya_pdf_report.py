"""
Vya PDF Report Generator (PDF1 + PDF2 + PDF3 combined).

COMPLETELY SEPARATE from the pre-order report and payment reports.
Same UI/styling, but different output file, separate backups, and ONLY
shows scenarios PDF1 through PDF3 (PDF generation scenarios — all 3
are part of the same overall PDF flow, combined into one report).

Output:
    Vya_PDF_Report.html

Usage:
    python vya_pdf_report.py
    from vya_pdf_report import build_report
"""
import json
import re
...
REPORT_PATH = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\Vya_PDF_Report.html")
BACKUP_DIR = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\.pdf_report_backups")

# ── Scenario tagging: PDF1 + PDF2 + PDF3 only ──────────────────────────────
PDF_NUM_PATTERN = re.compile(r'^PDF[123]$', re.IGNORECASE)


def is_pdf_scenario(record):
    """Returns True only for PDF1, PDF2, PDF3 scenarios."""
    num = (record.get("num") or "").strip()
    if not num:
        return False
    return bool(PDF_NUM_PATTERN.match(num))


def backup_history():
    """Save a timestamped copy of scenario_history.json to a PDF-only
    backup folder. Independent from other report backups."""
    if not HISTORY_PATH.exists():
        return None
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    dest = BACKUP_DIR / f"pdf_history_{stamp}.json"
    dest.write_bytes(HISTORY_PATH.read_bytes())
    backups = sorted(BACKUP_DIR.glob("pdf_history_*.json"))
    for old in backups[:-30]:
        try:
            old.unlink()
        except Exception:
            pass
    print(f"[Vya PDF Report] History backed up -> {dest.name}")
    return dest


# ── HTML template (identical UI to pre-order/payment reports) ─────────────
...
<head>
  ...
  <title>Vya PDF Automation - Test Report</title>
  <style>
...
def build_report(device="Android Phone", platform="Android"):
    """Build the PDF-only HTML report. Reads scenario_history.json,
    filters to PDF1/PDF2/PDF3 only, and writes Vya_PDF_Report.html."""
    try:
        backup_history()
    except Exception as _e:
        print(f"[Vya PDF Report] Backup skipped: {_e}")
    ...
    pdf_records = [r for r in all_records if is_pdf_scenario(r)]
    pdf_records = _dedupe_by_name(pdf_records)

    total = len(pdf_records)
    passed = sum(1 for r in pdf_records if r.get("status") == "PASS")
    failed = sum(1 for r in pdf_records if r.get("status") == "FAIL")
    pass_rate = round((passed / total) * 100) if total else 0

    rows_html = "\n        ".join(_build_row(r, i) for i, r in enumerate(pdf_records))
    if not rows_html:
        rows_html = ('<tr><td colspan="5" style="text-align:center;padding:40px;color:#999">'
                     'No PDF test results yet</td></tr>')
    ...
    print(f"[Vya PDF Report] Written {total} result(s) -> {REPORT_PATH}")
    return REPORT_PATH
