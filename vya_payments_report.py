"""
Vya Payments Report Generator (Set 1: PAY1-PAY8).
...
shows scenarios PAY1 through PAY8 (the original payment scenarios).

Output:
    Vya_Payments_Report.html

Usage:
    python vya_payments_report.py                  # build report from current history
    from vya_payments_report import build_report   # call programmatically
"""
import json
import re
...
REPORT_PATH = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\Vya_Payments_Report.html")
BACKUP_DIR = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\.payments_report_backups")

# ── Scenario tagging: PAY1 through PAY8 only ───────────────────────────────
PAYMENT_NUM_PATTERN = re.compile(r'^PAY[1-8]$', re.IGNORECASE)


def is_payment_scenario(record):
    """Returns True only for PAY1-PAY8 scenarios."""
...
def backup_history():
    """Save a timestamped copy of scenario_history.json to a payments-only
    backup folder. Independent from other report backups."""
    if not HISTORY_PATH.exists():
        return None
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    dest = BACKUP_DIR / f"payments_history_{stamp}.json"
    dest.write_bytes(HISTORY_PATH.read_bytes())
    backups = sorted(BACKUP_DIR.glob("payments_history_*.json"))
    for old in backups[:-30]:
        try:
            old.unlink()
        except Exception:
            pass
    print(f"[Vya Payments Report] History backed up -> {dest.name}")
    return dest
...
<head>
  ...
  <title>Vya Payments Automation - Test Report</title>
  <style>
...
def build_report(device="Android Phone", platform="Android"):
    """Build the Payments-only HTML report (PAY1-PAY8). Reads scenario_history.json,
    filters to PAY1-PAY8 only, and writes Vya_Payments_Report.html."""
    try:
        backup_history()
    except Exception as _e:
        print(f"[Vya Payments Report] Backup skipped: {_e}")
    ...
    if not rows_html:
        rows_html = ('<tr><td colspan="5" style="text-align:center;padding:40px;color:#999">'
                     'No payments test results yet</td></tr>')
    ...
    print(f"[Vya Payments Report] Written {total} result(s) -> {REPORT_PATH}")
    return REPORT_PATH
