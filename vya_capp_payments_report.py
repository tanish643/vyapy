"""
Vya C-App Payments Report Generator (Set 3: PAY12-PAY15 + REV1-REV3).
...
shows PAY12-15 (Host pays all / Cash / Food Voucher / Whom-to-pay)
and REV1-REV3 (review scenarios).

Output:
    Vya_CApp_Payments_Report.html

Usage:
    python vya_capp_payments_report.py
    from vya_capp_payments_report import build_report
"""
import json
import re
...
REPORT_PATH = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\Vya_CApp_Payments_Report.html")
BACKUP_DIR = Path(r"C:\Users\Tanish\Downloads\Vya-agentic-BOT (1)\Vya-agentic-BOT\.capp_payments_report_backups")

# ── Scenario tagging: PAY12-15 + REV1-3 ────────────────────────────────────
PAYMENT_NUM_PATTERN = re.compile(r'^(PAY(12|13|14|15)|REV[123])$', re.IGNORECASE)


def is_payment_scenario(record):
    """Returns True only for PAY12, PAY13, PAY14, PAY15, REV1, REV2, REV3."""
...
def backup_history():
    ...
    dest = BACKUP_DIR / f"capp_payments_history_{stamp}.json"
    dest.write_bytes(HISTORY_PATH.read_bytes())
    backups = sorted(BACKUP_DIR.glob("capp_payments_history_*.json"))
    for old in backups[:-30]:
        try:
            old.unlink()
        except Exception:
            pass
    print(f"[Vya C-App Payments Report] History backed up -> {dest.name}")
    return dest
...
<head>
  ...
  <title>Vya C-App Payments Automation - Test Report</title>
  <style>
...
<body>
  <div class="card">
    <div class="header">
      <h1>Vya C-App Payments Automation</h1>
...
def build_report(device="Android Phone", platform="Android"):
    """Build the C-App Payments HTML report (PAY12-15 + REV1-3)."""
    try:
        backup_history()
    except Exception as _e:
        print(f"[Vya C-App Payments Report] Backup skipped: {_e}")
    ...
    if not rows_html:
        rows_html = ('<tr><td colspan="5" style="text-align:center;padding:40px;color:#999">'
                     'No C-App payments test results yet</td></tr>')
    ...
    print(f"[Vya C-App Payments Report] Written {total} result(s) -> {REPORT_PATH}")
    return REPORT_PATH
